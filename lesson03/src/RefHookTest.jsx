...
//           useRef 는 input 요소에 사용 또는 단독을 이전값 사요을 위해서 정의
export function FocusInput() {
    // 컴포넌트
    const inputRef = useRef(null) // ⚡⚡참조 요소의 값이 바뀌어도 재렌더링 안함. nickname 재렌더링할 때 이전 값 유지

    // inputRef 와 비교하기 위해 정의
    const [nickname, setNickname] = useState('')    // ⚡⚡값이 바뀔때, 렌더링 다시 함.

    const handleFocus = () => {
        // 이벤트 함수
        inputRef.current.focus() // DOM 요소에 직접 접근(getElementById 와 동일한 동작)
...
export function PreviousValue() {
    ...
    // input 과 연결하지 않고 사용하는 예시. (value 속성 안씀. (value는 input에서만 사용하는 속성))
    const prevCount = useRef(0)

    useEffect(() => {
        // useRef 로 만들어진 변수는 객체. current 속성으로 값 설정.
        prevCount.current = count
    }, [count])     // count 가 바뀔떄마다 콜백함수 실행. [] 로 바꾸면 마운트할떄 한번 실행
